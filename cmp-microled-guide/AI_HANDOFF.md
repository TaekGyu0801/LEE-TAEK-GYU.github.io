...
## 2026-08-29 — ChatGPT · 사이트 논문 링크 및 초보자 가독성 개선

### 사용자 피드백

- 논문 제목을 누르면 PDF가 아니라 DOI/출판사 사이트가 열리는 문제가 있었음.
- P0~P9 Phase와 내부 소단원이 프로젝트를 처음 보는 사람에게는 용어 중심이라 이해하기 어려웠음.

### 확인한 원인

- `app.js`의 `paperLinks`가 R1~R10을 DOI/원문 랜딩 페이지 주소로 직접 덮어쓰도록 되어 있었음. 사용자 PDF를 검토해 둔 것과, GitHub Pages에서 그 PDF 파일 자체를 제공하는 것은 별개의 문제였음.
- ChatGPT File Library에 첨부된 PDF는 GitHub Pages의 정적 파일로 자동 복사되지 않음.

### 수정 내용

- `app.js`의 Phase 이름과 설명을 한국어 중심으로 재작성함.
- 각 Phase에 `쉽게 말하면`과 `이 단계가 끝나면` 설명을 추가함.
- P0–P2는 기준 소자 구축, P3–P6는 측벽 손상 모델링, P7–P9는 문헌 검증 및 실제 Wet/ALD 공정 연결이라는 큰 흐름을 Phase 목록 상단에 추가함.
- P0-01~P9-03 모든 소단원에 초보자용 한 줄 설명을 추가하고, 기존 summary는 `실제 작업`으로 분리함.
- CODE 배지는 `실행 코드 있음 / 실행 코드 아직 없음 / 앞 단계 코드 사용 / 실행 코드 없음`처럼 의미가 바로 보이게 바꿈.
- `index.html`의 연구 운영 규칙, 사이트 사용 방법, 결과값 설명, 문헌 역할 설명도 한국어 중심으로 단순화함.
- 브라우저가 이전 `app.js`를 캐시하지 않도록 script version을 `20260829-readable-pdf1`로 변경함.

### 논문 링크 변경

- R2/R3/R5/R6/R7/R8은 확인 가능한 출판사 직접 PDF 주소를 우선 링크함.
- R1은 GIST 공개 fulltext viewer를 우선 링크하고 논문 정보 페이지를 별도로 제공함.
- R4/R9/R10은 현재 안정적으로 확인한 공개 direct-PDF URL이 없어 DOI/출판사 원문 페이지 fallback을 유지함.
- 공개 GitHub에 사용자 첨부 PDF를 그대로 복제하는 방식은 라이선스가 명확한 문헌과 그렇지 않은 문헌을 구분해야 하므로 자동으로 수행하지 않음.

### GitHub 반영

- `app.js` commit: `0368a04a00d180859ba36924c5001447ae15dd30`
- `index.html` commit: `fec9da2505c95fe7abaf18b0498ae4a7bf303885`
